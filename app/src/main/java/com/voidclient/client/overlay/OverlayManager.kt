...
object OverlayManager {
    ...
    private fun initializeOverlays(context: Context) {
        ...
        when (theme) {
            GUITheme.CLASSIC -> {
                // YDK uses one floating circular launcher instead of per-module shortcut buttons.
    ...
    fun showOverlayWindow(overlayWindow: OverlayWindow) {
        if (overlayWindows.contains(overlayWindow)) {
            val context = currentContext
            if (isShowing && context != null && !isAttached(overlayWindow)) {
    ...
    fun toggleOverlayWindow(overlayWindow: OverlayWindow) {
        if (isAttached(overlayWindow)) dismissOverlayWindow(overlayWindow)
        else showOverlayWindow(overlayWindow)
    }

    private fun isAttached(overlayWindow: OverlayWindow): Boolean =
        try {
            overlayWindow.composeView.isAttachedToWindow
        } catch (_: Exception) {
            false
        }
    ...
    private fun showOverlayWindow(context: Context, overlayWindow: OverlayWindow) {
        ...
        try {
            ...
            // Overlay permission can be revoked while the app is running.
        }
    }
    ...
    // Backward-compatible name used by SettingsPage. YDK has a single circular launcher.
