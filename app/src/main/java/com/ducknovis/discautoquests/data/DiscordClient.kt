...
class DiscordClient(
        ...
        for (i in 0 until arr.length()) {
            ...
            val progress = taskType?.let {
                userStatus?.optJSONObject("progress")
                    ...
                    ?.optInt("value", 0) ?: 0
            } ?: userStatus?.optInt("stream_progress_seconds", 0) ?: 0
            ...
            // available task keys for debug
            val taskKeys = mutableListOf<String>()
            if (taskConfig != null) {
                val keys = taskConfig.keys()
                while (keys.hasNext()) taskKeys += keys.next()
            }
    ...
    /**
     * Heartbeat for PLAY / STREAM / ACTIVITY.
     * - PLAY_ON_DESKTOP: application_id + terminal
     * - STREAM / ACTIVITY: stream_key preferred (call:fake works for many accounts)
     */
    suspend fun postHeartbeat(
        ...
        val body = JSONObject().put("terminal", terminal)

        when (taskType) {
            TaskType.STREAM_ON_DESKTOP, TaskType.PLAY_ACTIVITY -> {
                // Fake stream key — many scripts use call:<channel>:<user>
                val streamKey = "call:1000000000000000000:1"
                body.put("stream_key", streamKey)
                ...
                // PLAY_ON_DESKTOP
                body.put("application_id", applicationId ?: JSONObject.NULL)
            }
        }

        val res = request("/quests/$questId/heartbeat", "POST", body) ?: return HeartbeatResult(false, 0)
        val json = JSONObject(res)
        val completed = json.has("completed_at") && !json.isNull("completed_at")

        // progress can be in different shapes
        var progressVal = 0
        val progressObj = json.optJSONObject("progress")
        if (progressObj != null && taskType != null) {
            progressVal = progressObj.optJSONObject(taskType.apiValue)?.optInt("value", 0) ?: 0
        }
        if (progressVal == 0) {
            progressVal = json.optInt("stream_progress_seconds", 0)
        }

        return HeartbeatResult(completed, progressVal)
    }
...
data class HeartbeatResult(val completed: Boolean, val progress: Int)
