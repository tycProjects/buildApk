...
class ProcessingWorker(appContext: Context, params: WorkerParameters) : CoroutineWorker(appContext, params) {
    ...
    private suspend fun synthesizeDubbedTrack(subtitles: List<SubtitleSegment>, totalDurationMs: Long, voiceTag: String): File? {
        val renderer = TtsRenderer(applicationContext)
        ...
        val clipFiles = mutableMapOf<Int, File>()
        subtitles.forEachIndexed { index, seg ->
            val text = seg.translation ?: seg.text
            if (text.isNotBlank()) {
                val out = File(cacheDir, "seg_$index.wav")
                if (renderer.renderToFile(text, out, locale)) clipFiles[index] = out
            }
        }
        ...
        cacheDir.deleteRecursively()
        return if (ok) composed else null
    }
