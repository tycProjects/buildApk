...
class SubtitleBitmapOverlay(
    ...
    private fun render(text: String): Bitmap {
        ...
        val fillPaint = Paint(Paint.ANTI_ALIAS_FLAG).apply {
            ...
            textSize = h * (style.fontSize / 420f).coerceIn(.025f, .10f)
            ...
            textAlign = when (style.align) { 0 -> Paint.Align.LEFT; 2 -> Paint.Align.RIGHT; else -> Paint.Align.CENTER }
            letterSpacing = style.letterSpacing
            setShadowLayer(style.shadow, 0f, style.shadow * .25f, Color.BLACK)
        }
        val strokePaint = Paint(fillPaint).apply {
            style = Paint.Style.STROKE
            strokeWidth = style.strokeWidth.coerceAtLeast(0f)
        ...
        val x = when (style.align) { 0 -> left + 8f; 2 -> right - 8f; else -> (left + right) / 2f }
        val baseY = bottom - (lines.size - 1) * lineHeight
        if (style.backgroundColor ushr 24 > 0) {
            val bg = Paint(Paint.ANTI_ALIAS_FLAG).apply { color = style.backgroundColor.toInt(); style = Paint.Style.FILL }
            canvas.drawRoundRect(left, top, right, bottom, style.backgroundRadius, style.backgroundRadius, bg)
        }
