 ...
 * 2) TikTok: tikwm.com
 * 3) CapCut: scrape preview video từ trang template
 * 4) Cobalt community instances (Bilibili + đa nền tảng)
 * 5) oEmbed fallback (title + thumbnail)
 */
class VideoExtractor(
    ...
    suspend fun extract(url: String): Result<VideoInfo> = withContext(Dispatchers.IO) {
        ...
        when (platform) {
            ...
            Platform.CAPCUT -> {
                extractCapCut(cleanUrl)?.let { return@withContext Result.success(it) }
                    ?: errors.add("CapCut")
            }
            else -> { }
        }
    ...
    fun detectPlatform(url: String): Platform {
        ...
        return when {
            ...
            lower.contains("bilibili.com") || lower.contains("bilibili.tv") ||
                lower.contains("b23.tv") || lower.contains("bili2233.cn") -> Platform.BILIBILI
            lower.contains("capcut.com") || lower.contains("capcut.net") -> Platform.CAPCUT
            else -> Platform.OTHER
        }
    }
    ...
    private fun extractCapCut(url: String): VideoInfo? {
        return try {
            val req = Request.Builder()
                .url(url)
                .header("User-Agent", USER_AGENT)
            ...
            client.newCall(req).execute().use { resp ->
                ...
                if (!ogTitle.isNullOrBlank()) title = ogTitle.trim()
                ...
                val videoPatterns = listOf(
                    Regex("""https?://v\d+-capcut[^"'\s<>]+"""),
                    Regex("""https?://[^"'\s<>]*capcutcdn[^"'\s<>]+\.(?:mp4|m3u8)[^"'\s<>]*"""),
                    Regex("""https?://[^"'\s<>]*capcutcdn[^"'\s<>]*/video/[^"'\s<>]+"""),
                    Regex(""""playAddr"\s*:\s*"([^"]+)""""),
                    Regex(""""video_url"\s*:\s*"([^"]+)""""),
                    Regex(""""videoUrl"\s*:\s*"([^"]+)""""),
                ...
                for (pat in videoPatterns) {
                    for (m in pat.findAll(html)) {
                        var vurl = if (m.groupValues.size > 1) m.groupValues[1] else m.value
                        vurl = vurl
                            .replace("\\u002F", "/")
                            .replace("\\/", "/")
                            .replace("&amp;", "&")
                        ...
                        if (vurl in seen) continue
                        if (vurl.contains("tplv-") &&
                            (vurl.contains("image") || vurl.endsWith(".webp") ||
                                vurl.endsWith(".jpg") || vurl.endsWith(".png"))
                        ) continue
                        seen.add(vurl)
                        val format = if (vurl.contains(".m3u8")) "m3u8" else "mp4"
                        qualities.add(VideoQuality("HD", format, null, vurl, true))
                    }
                    if (qualities.isNotEmpty()) break
                }

                if (qualities.isEmpty()) return@use null

                VideoInfo(
                    title = title,
                    thumbnailUrl = thumb,
                    duration = 0,
                    platform = Platform.CAPCUT,
                    originalUrl = url,
                    author = null,
                    qualities = qualities
                )
