...
public class AutoTyperService extends AccessibilityService {
    ...
    private static boolean pendingAutoRepeat = false;

    /** Gọi từ MainActivity trước khi broadcast ACTION_START */
    public static void setPendingMessages(List<String> messages, int delayMs, int charDelayMs, boolean pasteMode, boolean autoRepeat) {
        pendingMessages = messages;
        pendingDelayMs = delayMs;
        pendingCharDelayMs = charDelayMs;
        pendingPasteMode = pasteMode;
        pendingAutoRepeat = autoRepeat;
    }
    ...
    private boolean lastAutoRepeat = false;
    private boolean autoRepeat = false;

    // Đánh dấu đã ghi log chờ bàn phím (tránh spam)
    private boolean waitingKeyboardLogged = false;
    ...
    private final Runnable singleVolumeDownAction = () -> {
        ...
        if (running) {
            pauseTyping();
            Log.d(TAG, "Paused by Volume Down");
        }
    };
            ...
            public void onReceive(Context context, Intent intent) {
                ...
                if ("com.ryan.autotyper.ACTION_START".equals(action)) {
                    ...
                    if (pendingMessages != null && !pendingMessages.isEmpty()) {
                        ...
                        boolean repeat = pendingAutoRepeat;
                        ...
                        lastPasteMode = paste;
                        lastAutoRepeat = repeat;

                        sessionActive = true;
                        startTyping(msgs, delay, charDelay, paste, repeat);
                    }
    ...
    protected boolean onKeyEvent(KeyEvent event) {
        ...
        if (keyCode == KeyEvent.KEYCODE_VOLUME_UP) {
            if (!running) {
                if (messages != null && currentIndex < messages.size()) {
                    running = true;
                    handler.post(typingRunnable);
                    Log.d(TAG, "Resumed by Volume Up");
                } else if (lastMessages != null && !lastMessages.isEmpty()) {
                    startTyping(lastMessages, lastDelayMs, lastCharDelayMs, lastPasteMode, lastAutoRepeat);
                    Log.d(TAG, "Loop restarted by Volume Up");
                }
            }
            return true;
        }
        ...
        if (keyCode == KeyEvent.KEYCODE_VOLUME_DOWN) {
            if (volumeDownPending) {
                ...
                Log.d(TAG, "Floating menu toggled");
            } else {
    ...
    private void startTyping(List<String> msgs, int delay, int charDelay, boolean paste, boolean repeat) {
    ...
    private final Runnable typingRunnable = new Runnable() {
        ...
        public void run() {
            ...
            if (!isKeyboardOpen()) {
                if (!waitingKeyboardLogged) {
                    Log.d(TAG, "Waiting for keyboard");
                    waitingKeyboardLogged = true;
                }
    ...
    private void finishOrRepeat(String mode) {
        if (autoRepeat && messages != null && !messages.isEmpty()) {
            currentIndex = 0;
            currentMessage = "";
            charIndex = 0;
            typedSoFar.setLength(0);
            running = true;
            Log.d(TAG, "Finished " + mode + "; repeating automatically");
            handler.postDelayed(typingRunnable, Math.max(200, delayMs));
        } else {
            running = false;
            Log.d(TAG, "Finished all messages (" + mode + ")");
        }
    }
